      ...
      {loadingOneMemberFinishedSchedules ? (
        ...
        <>
          ...
          <Heading
            ...
            fontSize="20px"
            mb={10}
            data-testid="one-member-detail-nickname"
          >
          ...
          {dataMyTeamTrainings?.myTeamTrainings.edges.length === 0 && (
            <Center mt={10}>トレーニングはありません。</Center>
          )}
          {dataMyTeamTrainings?.myTeamTrainings.edges?.map((train) => (
            <Box key={train.node.id} borderBottom="1px solid #718096" py={3}>
              <Flex alignItems="center">
                ...
                <Text
                  w="250px"
                  data-testid={train.node.id + '-training-contents'}
                >
                  {train.node.title}
                </Text>
                <Text
                  w="50px"
                  pl="10px"
                  data-testid={train.node.id + '-finished-count'}
                >
                  {
          ...
          <TrainingImplementationModal
            finishedSchedules={
              dataOneMemberFinishedSchedules.myTeamFinishedSchedules
            }
          />
        </>
      )}

...
export const MyTeamMemberListSection: VFC = memo(() => {
  ...
  const {
    ...
    fetchMore,
  } = useGetMoreMyTeamMember()

  const onChangeTentativeNickname = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => setTentativeNickname(e.target.value),
    []
  )
  ...
  const fetchSearchMember = useCallback((nickname: string) => {
    fetchMore({
      variables: {
        nickname,
        first: NUM_PAGE,

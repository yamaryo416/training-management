                ...
                <>
                    ...
                    <Stack spacing={5} mb={10}>
                        <Flex>
                            ...
                            <Text pl={3} data-testid='selected-schedule-finished-title'>{title}</Text>
                        </Flex>
                        <Flex>
                    ...
                    <Box mb={10}>
                        {dataMyTeamMember?.myTeamMember.edges?.filter(({ node }) => scheduleFinishedMemberId!.includes(node.id)).
                            map(({ node }) => (
                                <Box key={node.id}>
                                    <FinishedScheduleMemberItem nickname={node.nickname} isFinished={true} />
                                </Box>
                            ))
                        }
                    </Box>
                    ...
                    <Box>
                        {dataMyTeamMember?.myTeamMember.edges?.filter(({ node }) => !scheduleFinishedMemberId!.includes(node.id)).
                            map(({ node }) => (
                                <Box key={node.id}>
                                    <FinishedScheduleMemberItem nickname={node.nickname} isFinished={false} />
                                </Box>
                            ))
                        }
                    </Box>

...
export const TrainingImplementationModal: VFC<Props> = memo((props) => {
  ...
  const trainingImplementationModal = useRecoilValue(
    trainingImplementationState
  )

  const { trainingId, title, finishedPatern, isOpen } =
    trainingImplementationModal

  const trainingTotalNubmer = useCallback(() => {
    const number = [0, 0, 0]
    if (finishedPatern.includes('1')) {
      finishedSchedules.edges
        .filter((fSche) => fSche.node.training.id === trainingId)
        .map((fsche) => {
  ...
  return (
    ...
    >
      <Flex pt={5}>
        ...
        <Text pl={2} data-testid="training-implementation-title">
          {title}
        </Text>
      </Flex>
      <Flex>
        ...
        {finishedPatern.includes('1') && (
          <Text pl={2}>{trainingTotalNubmer()[0]} 回</Text>
        )}
        {finishedPatern.includes('3') && (
          <Text pl={2}>{trainingTotalNubmer()[1]} km</Text>
        )}
        {finishedPatern.includes('4') && (
          <Text pl={2}>{trainingTotalNubmer()[2]} 分</Text>
        )}
        <Text pl={2}>実施</Text>
      </Flex>
      <Box>

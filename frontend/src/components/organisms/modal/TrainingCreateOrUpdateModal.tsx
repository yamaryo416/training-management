...
export const TrainingCreateOrUpdateModal:VFC = memo(() => {
    ...
    const { id, title, description, iconNumber, finishedPatern, isCreate, isOpen } = trainingCreateOrUpdateModal
    ...
    const onChangeFinishedPatern = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value
        if (finishedPatern.includes(value)) {
            setTrainingCreateOrUpdateModal({ ...trainingCreateOrUpdateModal, finishedPatern: finishedPatern.replace(value, '') })
        } else {
            setTrainingCreateOrUpdateModal({ ...trainingCreateOrUpdateModal, finishedPatern: finishedPatern + value })
        }
    }, [trainingCreateOrUpdateModal])
    ...
    return (
        ...
        >
            <Formik
                ...
                onSubmit={(values) => {
                    isCreate ?
                    createTraining(values.title, values.description, undefinedNumber(iconNumber), finishedPatern ):
                    updateTraining(id, values.title, values.description, undefinedNumber(iconNumber), finishedPatern)
                }}
                  ...
                  {({
                    ...
                    <form onSubmit={handleSubmit}>
                        <Stack spacing={4}>
                            ...
                            <Box>
                                <FormLabel fontSize="20px">トレーニングを実施した際に<br/>入力する項目</FormLabel>
                                <Flex>
                                    <Box mr={2}>
                                        <input type="checkbox" value='1' checked={finishedPatern.includes('1')} data-testid='finished-patern-1' onChange={onChangeFinishedPatern}/>
                                    </Box>
                                    <label>回数</label>
                                </Flex>
                                <Flex>
                                    <Box mr={2}>
                                        <input type="checkbox" value='2' checked={finishedPatern.includes('2')} data-testid='finished-patern-2' onChange={onChangeFinishedPatern}/>
                                    </Box>
                                    <label>kg</label>
                                </Flex>
                                <Flex>
                                    <Box mr={2}>
                                        <input type="checkbox" value='3' checked={finishedPatern.includes('3')} data-testid='finished-patern-3' onChange={onChangeFinishedPatern}/>
                                    </Box>
                                    <label>km</label>
                                </Flex>
                                <Flex>
                                    <Box mr={2}>
                                        <input type="checkbox" value='4' checked={finishedPatern.includes('4')} data-testid='finished-patern-4' onChange={onChangeFinishedPatern}/>
                                    </Box>
                                    <label>分</label>
                                </Flex>
                            </Box>

...
export const ConfirmFinishedScheduleDeleteModal: VFC = memo(() => {
    ...
    const confirmFinishedScheduleDeleteModal = useRecoilValue(confirmFinishedScheduleDeleteModalState)
    ...
    return (
        ...
        >
            <Heading as="h3" fontSize="20px">以下のスケジュール実施を取り消します。<br/>よろしいですか？</Heading>
                <Flex>
                    <Heading fontSize='18px'>スケジュール:</Heading>
                    <Text pl={2} fontSize='16px' data-testid='finished-schedule-delete-title'>{title}</Text>
                </Flex>
                <Flex>
                    <Heading fontSize='18px'>日付:</Heading>
                    <Text pl={2} fontSize='16px' data-testid='finished-schedule-delete-date'>{moment(date).format('YYYY年M月D日')}</Text>
                </Flex>

      ...
      {loadingMyTeamFinishedSchedules || loadingMyTeamMember ? (
        ...
        <>
          {errorMyTeamFinishedSchedules || errorMyTeamMember ? (
            ...
            <Stack spacing={10}>
              <Flex>
                <Heading fontSize="16px">スケジュール:</Heading>
                <Text pl={2}>{title}</Text>
              </Flex>
              <Flex>
                <Heading fontSize="16px">日付:</Heading>
                <Text pl={2}>{moment(date).format('YYYY年M月D日(ddd)')}</Text>
              </Flex>
              ...
              <Box>
                {dataMyTeamMember?.myTeamMember.edges
                  ?.filter(
                    (member) =>
                      !scheduleFinishedMemberId!.includes(member.node.id)
                  )
                  .map(({ node }) => (
                    <FinishedScheduleMemberItem
                      isFinished={false}

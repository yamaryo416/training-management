  ...
  return (
    ...
    >
      <Box textAlign="center">
        <FontAwesomeIcon icon={faCircleUser} style={{ fontSize: '100px' }} />
      </Box>
      <Heading as="h4" textAlign="center" fontSize="16px" mb={10}>
        {oneMemberSelectedModal.nickname}
      </Heading>
      ...
      <Heading as="h4" textAlign="center" fontSize="16px">
        各トレーニングの実施状況
      </Heading>
      {loadingMyTeamTrainings ||
        (loadingOneMemberFinishedSchedules && <CustomSpinner />)}
            ...
            {dataMyTeamTrainings?.myTeamTrainings.edges?.map((train) => (
              <Box key={train.node.id} borderBottom="1px solid #718096">
                <Flex alignItems="center" py={2}>
                  <Box w="50px">
                    <TrainingIcon
                      iconNumber={train.node.iconNumber}
                      color="black"
                      size="50px"
                    />
                  </Box>
                  <Text w="180px">{train.node.title}</Text>
                  <Text w="50px" pl="20px">
                    {
                      dataOneMemberFinishedSchedules?.myTeamFinishedSchedules.edges?.filter(
                        (sche) => sche.node.training.id === train.node.id
                      ).length
                    }
                    回
                  </Text>
